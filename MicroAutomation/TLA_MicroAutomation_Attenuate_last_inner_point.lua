-- @name TLA_MicroAutomation_Attenuate_last_inner_point
-- @about 
--   Attenuate the last inner point of a four-point envelope.
...
function TLA.performAction()
  ...
  if envelopeProperties.armed and envelopeProperties.visible then
    ...
    local valueAtLastPoint = TLA.getEnvelopeValueAtTime(envelope, lastPointOverall.time)
    ...
    local finalValue = TLA.calculateFinalPointValue(valueAtLastPoint,
      adjustedMidiInputValue,
      envelopeProperties.minValue,
      envelopeProperties.maxValue,
      TLA.config.maxRange)
    log(' -finalValue: ' .. finalValue)
    TLA.setPointValue(envelope, lastInner, finalValue)
