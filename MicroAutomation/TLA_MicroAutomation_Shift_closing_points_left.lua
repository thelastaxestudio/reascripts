-- @name TLA_MicroAutomation_Shift_closing_points_left
-- @about 
--   Bump closing points of a four-point selection to the left by 0.01 seconds.
--   Meant to be used with midi pad input.
...
function TLA.performAction()
  ...
  if envelopeProperties.armed and envelopeProperties.visible then
    ...
    -- shuffle the inner point first so there isn't point confusion
    TLA.setPointTime(envelope, lastPointInner, lastPointInner.time - TLA.config.increments)
    TLA.setPointTime(envelope, lastPointOuter, lastPointOuter.time - TLA.config.increments)
